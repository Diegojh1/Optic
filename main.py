...
TTS_ENABLED = True  # Activar/desactivar TTS
TTS_MODE = "hybrid"  # "pyttsx3", "gtts", "hybrid"
TTS_SPEED = 150  # Palabras por minuto
TTS_VOLUME = 0.8  # Volumen TTS (0.0 - 1.0)
TTS_LANGUAGE = "es"  # Idioma (español)
TTS_VOICE_GENDER = "female"  # "male", "female"
tts_engine = None  # Instancia del motor TTS
...
def init_tts_engine():
    """Inicializa el motor TTS offline."""
    global tts_engine
    ...
    try:
        if tts_engine is None:
            ...
            tts_engine.setProperty('rate', TTS_SPEED)
            ...
            for voice in voices:
                ...
                if any(indicator in voice_name or indicator in voice_id 
                       for indicator in ['spanish', 'es', 'esp', 'méxico', 'mexico', 'colombia']):
                    spanish_voice = voice.id
                    
                    # Preferir voces femeninas si se solicita
                    if (TTS_VOICE_GENDER == "female" and 
                        any(fem in voice_name for fem in ['female', 'mujer', 'helena', 'maria', 'sofia'])):
                        break
                    elif (TTS_VOICE_GENDER == "male" and 
                          any(masc in voice_name for masc in ['male', 'hombre', 'diego', 'carlos', 'juan'])):
                        break
            
            if spanish_voice:
                tts_engine.setProperty('voice', spanish_voice)
                print(f"✅ Voz TTS configurada: {spanish_voice}")
            else:
...
def speak_offline(text):
    ...
    try:
        ...
        print(f"❌ Error TTS offline: {e}")
        return False

def speak_online(text):
    """TTS online usando gTTS."""
    try:
        ...
        temp_file = os.path.join(DATA_DIR, "temp_tts.mp3")
        tts.save(temp_file)
        ...
        if pygame.mixer.get_init():
            ...
            # Esperar a que termine y limpiar archivo
        ...
        print(f"❌ Error TTS online: {e}")
        return False

def speak_text(text, mode="auto"):
    """
    Función principal para convertir texto a voz.
    
    Args:
        text (str): Texto a pronunciar
        mode (str): "auto", "offline", "online", "pyttsx3", "gtts"
    """
    global TTS_ENABLED, TTS_MODE
    
    if not TTS_ENABLED or not text or len(text.strip()) == 0:
        return
    
    # Limpiar texto para TTS
    clean_text = text.strip()
    
    # Remover marcadores de tiempo y fuente si existen
    if "] " in clean_text and "[" in clean_text:
        # Formato: "[12:34:56] gesto (entrenado 85%)"
        clean_text = clean_text.split("] ", 1)[-1]
    
    # Remover información de fuente al final
    if " (" in clean_text and clean_text.endswith(")"):
        clean_text = clean_text.split(" (")[0]
    
    # Decidir modo si es automático
    if mode == "auto":
        if TTS_MODE == "hybrid":
            # Intentar online primero si hay internet, sino offline
            has_internet = check_internet_connection()
            mode = "online" if (has_internet and GTTS_AVAILABLE) else "offline"
        else:
            mode = TTS_MODE
    
    # Mapear modos alternativos
    if mode == "pyttsx3":
        mode = "offline"
    elif mode == "gtts":
        mode = "online"
    ...
    try:
        if mode == "online":
            print(f"🔊 TTS Online: '{clean_text}'")
            success = speak_online(clean_text)
            
            if not success and PYTTSX3_AVAILABLE:
                print("🔄 Fallback a TTS offline")
                success = speak_offline(clean_text)
                
        elif mode == "offline":
            print(f"🔊 TTS Offline: '{clean_text}'")
            success = speak_offline(clean_text)
            
        if not success:
            print(f"❌ No se pudo reproducir TTS: '{clean_text}'")
            
    except Exception as e:
        print(f"❌ Error general TTS: {e}")

def speak_text_async(text, mode="auto"):
    """Versión asíncrona de speak_text para no bloquear la UI."""
    if TTS_ENABLED:
        tts_thread = threading.Thread(target=speak_text, args=(text, mode), daemon=True)
        tts_thread.start()
...
def hybrid_gesture_recognition(frame, hand_landmarks, face_landmarks=None):
    ...
    try:
        ...
        if final_result:
            ...
            # NUEVO: Agregar TTS para leer la traducción
            speak_text_async(current_translation)
...
class SignLanguageTranslatorApp(ctk.CTk):
    ...
    def create_settings_frame(self):
        ...
        # Sección: Configuración de TTS (Texto a Voz)
        self.create_settings_section(
            self.settings_scroll, 
        ...
        # Marco para controles TTS
        self.tts_frame = ctk.CTkFrame(self.settings_scroll, fg_color="#101830")
        self.tts_frame.grid(row=9, column=0, padx=20, pady=10, sticky="ew")
        self.tts_frame.grid_columnconfigure(1, weight=1)
        
        # Variable para TTS habilitado
        self.tts_var = tk.IntVar(value=1 if TTS_ENABLED else 0)
        
        self.tts_checkbox = ctk.CTkCheckBox(
            self.tts_frame,
            text="Activar Texto a Voz",
            command=self.toggle_tts,
            variable=self.tts_var,
            font=("Helvetica", 12)
        )
        self.tts_checkbox.grid(row=0, column=0, columnspan=3, padx=10, pady=5, sticky="w")
        
        # Velocidad TTS
        self.tts_speed_label = ctk.CTkLabel(
            self.tts_frame, 
            text="Velocidad de Voz:",
            font=("Helvetica", 12)
        )
        self.tts_speed_label.grid(row=1, column=0, padx=10, pady=5, sticky="w")
        ...
        self.tts_speed_slider.grid(row=1, column=1, padx=10, pady=5, sticky="ew")
        self.tts_speed_slider.set(TTS_SPEED)
        
        self.tts_speed_value = ctk.CTkLabel(
            self.tts_frame,
            text=f"{TTS_SPEED} WPM",
            width=80
        )
        self.tts_speed_value.grid(row=1, column=2, padx=10, pady=5, sticky="w")
        ...
        self.tts_volume_label.grid(row=2, column=0, padx=10, pady=5, sticky="w")
        ...
        self.tts_volume_slider.grid(row=2, column=1, padx=10, pady=5, sticky="ew")
        self.tts_volume_slider.set(TTS_VOLUME)
        
        self.tts_volume_value = ctk.CTkLabel(
            self.tts_frame,
            text=f"{int(TTS_VOLUME * 100)}%",
            width=80
        )
        self.tts_volume_value.grid(row=2, column=2, padx=10, pady=5, sticky="w")
        
        # Modo TTS
        self.tts_mode_label = ctk.CTkLabel(
            self.tts_frame, 
            text="Modo de Voz:",
            font=("Helvetica", 12)
        )
        self.tts_mode_label.grid(row=3, column=0, padx=10, pady=5, sticky="w")
        
        # Variable para el modo TTS
        self.tts_mode_var = tk.StringVar(value=TTS_MODE)
        
        self.tts_mode_option = ctk.CTkOptionMenu(
            self.tts_frame,
            values=["hybrid", "pyttsx3", "gtts"],
            command=self.update_tts_mode,
            variable=self.tts_mode_var,
            width=150
        )
        self.tts_mode_option.grid(row=3, column=1, padx=10, pady=5, sticky="w")
        ...
        self.test_tts_btn.grid(row=3, column=2, padx=10, pady=5)
        ...
        self.trained_gestures_frame.grid(row=11, column=0, padx=20, pady=10, sticky="ew")
        ...
        self.sensitivity_frame.grid(row=12, column=0, padx=20, pady=10, sticky="ew")
        ...
        self.cooldown_frame.grid(row=13, column=0, padx=20, pady=10, sticky="ew")
        ...
        self.save_config_btn.grid(row=14, column=0, padx=20, pady=30)
...
🔊 Texto a Voz (TTS)
- pyttsx3 - Motor TTS offline multiplataforma
- gTTS (Google Text-to-Speech) - TTS online de alta calidad
- Sistema híbrido con fallback automático
- Voces configurables en español colombiano
...
- Conexión a Internet: 5 Mbps mínimo (para TTS online)
...
- Sistema de Texto a Voz integrado con múltiples modos
...
- Interfaz de usuario intuitiva y moderna
    ...
    def translate_text_to_sign(self):
        ...
        try:
            ...
            if gif_file:
                ...
                # NUEVO: Leer el texto ingresado antes de mostrar la seña
    ...
    def toggle_tts(self):
        """Activa/desactiva el sistema TTS."""
        global TTS_ENABLED
        TTS_ENABLED = bool(self.tts_var.get())
        print(f"🔊 TTS: {'ACTIVADO' if TTS_ENABLED else 'DESACTIVADO'}")
    
    def update_tts_speed(self, value):
        """Actualiza la velocidad de TTS."""
        global TTS_SPEED
        TTS_SPEED = int(value)
        self.tts_speed_value.configure(text=f"{TTS_SPEED} WPM")
        
        # Actualizar motor TTS si existe
        if tts_engine:
            try:
                tts_engine.setProperty('rate', TTS_SPEED)
            except:
    ...
    def update_tts_volume(self, value):
        """Actualiza el volumen de TTS."""
        global TTS_VOLUME
        TTS_VOLUME = float(value)
        self.tts_volume_value.configure(text=f"{int(TTS_VOLUME * 100)}%")
    ...
    def update_tts_mode(self, value):
        """Actualiza el modo de TTS."""
        global TTS_MODE
        TTS_MODE = value
        print(f"🔊 Modo TTS cambiado a: {TTS_MODE}")
    
    def test_tts(self):
        """Prueba el sistema TTS con un mensaje de ejemplo."""
        test_messages = [
            "Hola, soy OPTIC, tu asistente de lenguaje de señas",
            "Sistema de texto a voz funcionando correctamente",
            "Bienvenido al traductor OPTIC"
        ]
        
        import random
        test_text = random.choice(test_messages)
        
        print(f"🧪 Probando TTS: '{test_text}'")
        speak_text_async(test_text)
...
def load_config():
    ...
    global TTS_ENABLED, TTS_MODE, TTS_SPEED, TTS_VOLUME, TTS_LANGUAGE, TTS_VOICE_GENDER
    ...
    if os.path.exists(config_path):
        try:
            ...
            # Cargar configuraciones TTS
            TTS_ENABLED = config.get("tts_enabled", TTS_ENABLED)
            TTS_MODE = config.get("tts_mode", TTS_MODE)
            TTS_SPEED = config.get("tts_speed", TTS_SPEED)
            TTS_VOLUME = config.get("tts_volume", TTS_VOLUME)
            TTS_LANGUAGE = config.get("tts_language", TTS_LANGUAGE)
            TTS_VOICE_GENDER = config.get("tts_voice_gender", TTS_VOICE_GENDER)
...
def main():
    ...
    try:
        ...
        print("🔊 Inicializando sistema de Texto a Voz...")
        init_tts_engine()
        if TTS_ENABLED:
            print(f"✅ TTS activado - Modo: {TTS_MODE}")
        else:
